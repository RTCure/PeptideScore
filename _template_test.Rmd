...
```{targets ws.file}
tar_target(ws.file,
             "/projects/cytofResults/collaborations/RTCure/analysis/ks_files\ /20211025_METACIR_ROUND2/data/130122_above10_noagg.wsp",
             format = "file")
```
...
"/projects/cytofResults/collaborations/RTCure/analysis/ayann/WP4/PeptideScore/input"
```
...
.annots <- pData(gset) %>%
      ...
      mutate(Treatment = str_extract(`FCS Filename`,
                                     "(CD3|CEFT|CEF|TT|HEX|CVIM|CENO|CCILP|CENO|CFIB)"),
             Individual = str_extract(`FCS Filename`,
                                      "(MET121|MET123|MET101|MET124|MET129|MET132|MET142|MET111|MET114|MET127|MET134|MET136|MET139|MET152|MET153|MET154|MET156|MET170)"),
             Treatment = factor(Treatment),
             Individual = factor(Individual),
             Treatment = fct_relevel(Treatment,"CD3","CEFT"),
             Individual = fct_relevel(Individual,"MET121","MET123"),
...
"/Cells/Single Cells/Live cells /CD4+ T cells/New divided"
```
...
tar_target(div.df,{
  ...
  .div.df <- as_tibble(fsApply(.div.df,exprs)) %>% 
  ...
  .cd3.df <- .div.df %>% 
    ...
    group_by(Individual) %>% 
    sample_n(500) # sample 500 cells from each donor
  
  .div.df <- bind_rows(.cd3.df,
                       .div.df %>% 
                         filter(Treatment!="CD3"))
})
```
...
Using all functional markers in the dataset we compute the cosine similarity between all cells:
